...
此页面让你概述了BPMN 2.0元素和流程引擎的当前实现范围。

{{< note title="BPMN - 业务流程模型和符号" >}}
如果你不熟悉BPMN 2.0，你可能想先看看
...
在BPMN中，有开始事件，中间事件和结束事件。这三种事件类型都可以捕捉事件或抛出事件。中间事件可以用作任务上的里程碑事件，在这种情况下，它们可以是中断或非中断的。在你的流程中使用事件可以为你提供了许多灵活性。
